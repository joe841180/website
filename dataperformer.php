...
// step3: 執行SQL
$sql  = "select * from  performer where	id like '%$key%' or  name like '%$key%' or  country like '%$key%' or  gender like '%$key%' or  age like '%$key%'";    //關鍵字需要查找的欄位

if ($field == NULL){
$sql  = "select * from  performer where	id like '%$key%' or  name like '%$key%' or  country like '%$key%' or  gender like '%$key%' or  age like '%$key%'";
}
else if($field != NULL){
$sql = "select* from performer where $field like '%$key%'";
}
...
echo "<table width='700' border='1' rules='all' cellpadding='5' align='center'; >" ;
  ...
  echo "<th>年齡</th>";
    ...
    while($row !=NULL){  
   list($id,$name,$country,$gender,$age) = $row ;
